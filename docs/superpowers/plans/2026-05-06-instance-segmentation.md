...
| `analysis/visualize_pred.py` | Interactive matplotlib submission viewer (browse test-results.json on test images) |
...
## Task 6b: Submission visualizer

**Files:**
- Create: `analysis/visualize_pred.py`

Same keyboard navigation as `visualize_gt.py`. Images are test images from `data/test_release/`, sorted alphabetically by filename. Predictions loaded from `test-results.json` (or any path passed via `--results`).

- [ ] **Step 6b.1: Write `analysis/visualize_pred.py`**
...
class PredViewer:
    def __init__(self, test_dir: Path, results_path: Path, id_map_path: Path) -> None:
        ...
        self.id_to_hw: dict[int, tuple[int, int]] = {
            e["id"]: (e["height"], e["width"]) for e in id_list
        }
        # Sort test images alphabetically — same order as GT viewer convention
    ...
    def _render(self) -> None:
        ...
        # Sort by score descending so higher-confidence masks are drawn on top
...
- [ ] **Step 6b.2: Run after inference to visually verify predictions**

```bash
uv run python analysis/visualize_pred.py --results test-results.json
```

Check for obvious issues:
- [ ] Masks appear on the correct images (not offset by one)
- [ ] Category labels match expected cell types
- [ ] Scores are in (0, 1] range
- [ ] No masks wildly outside the image bounds

- [ ] **Step 6b.3: Commit**

```bash
git add analysis/visualize_pred.py
git commit -m "feat: interactive submission/prediction visualizer"
```
...
| Submission visualizer (test-results.json on test images) | `analysis/visualize_pred.py` Task 6b |
