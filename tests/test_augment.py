...
def _make_sample(H=64, W=64, n_inst=3):
    ...
    boxes = torch.tensor(
        [[5.0, 5.0, 20.0, 20.0], [30.0, 30.0, 50.0, 50.0], [10.0, 40.0, 40.0, 60.0]]
    )[:n_inst]
...
def test_val_transform_no_spatial_change():
    ...
    t = get_val_transform()
    img_in = tv_tensors.Image(
        torch.arange(0, 3 * 4 * 4).reshape(3, 4, 4).to(torch.uint8)
    )
    ...
    target = {
        ...
        "masks": tv_tensors.Mask(torch.ones(1, 4, 4, dtype=torch.uint8)),
    }
